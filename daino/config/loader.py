...
from daino.config.globals import GLOBAL_SECTIONS, load_global_data, merge_layers
from daino.config.models import Settings
from daino.exceptions import ConfigurationError

#: Legacy alias kept for callers/tests that import ``CONFIG_DIR``; the resolver
#: in :mod:`daino.config.paths` is authoritative and handles the ``.vasuki``
#: fallback.
CONFIG_DIR = paths.STATE_DIR
...
def find_project_root(start: Path | None = None) -> Path:
    ...
    the user actually chose. It must not be replaced by a parent Git/Daino
...
def config_path(root: Path | None = None) -> Path:
    return paths.state_dir(find_project_root(root)) / CONFIG_FILE
...
def load_settings(root: Path | None = None, *, require: bool = True) -> Settings:
    ...
    if not path.exists():
        if require and not global_data:
            raise ConfigurationError(f"No {CONFIG_DIR}/{CONFIG_FILE}; run `daino init` first")
