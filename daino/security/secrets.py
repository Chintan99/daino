...
def resolve_secret(reference: str) -> str:
    ...
    if reference.startswith("keyring://"):
        try:
            ...
            raise ConfigurationError("Install daino[keyring] to use keyring secrets") from exc
...
def store_global_secret(name: str, value: str) -> str:
    ...
    """
    from daino.config.globals import global_config_dir

    return store_project_secret(global_config_dir(), name, value, marker="")


def store_project_secret(root: Path, name: str, value: str, *, marker: str | None = None) -> str:
    """Store a validated secret in a private project file and return only its reference.

    With ``marker`` unset the project state directory is resolved via
    :mod:`daino.config.paths` (``.daino`` for new projects, legacy ``.vasuki``
    when present). ``marker=""`` stores directly under ``root`` (used for global
    secrets); an explicit marker names a fixed subdirectory.
    """
    ...
    if marker is None:
        state_directory = paths.state_dir(root.resolve(), create=True)
    elif marker:
        state_directory = (root.resolve() / marker).resolve()
    else:
        state_directory = root.resolve()
