...
from daino.runtimes.detect import preferred_runtime
...
def _ensure_git_baseline(root: Path) -> None:
    ...
    if not owns_repository:
        ...
        # parent repository. Initializing here is intentional: Daino state,
...
def initialize_project(root: Path, *, force: bool = False) -> InitializationResult:
    """Initialize a project using the same operations as ``daino init``."""
    root = root.resolve()
