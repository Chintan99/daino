...
import pytest
from textual.widgets import ContentSwitcher, Input, ListView, Select
...
from vasuki.tui.screens.views import ProvidersView
...
async def test_command_palette_and_narrow_layout(tmp_path: Path) -> None:
    ...
    async with app_instance.run_test(size=(68, 28)) as pilot:
        ...
        # The single-column layout survives a narrow terminal; only the serpent
        # art drops away, and the chat and prompt keep their full width.
        assert workspace.has_class("narrow")
        assert not workspace.query_one("#welcome-art").display
