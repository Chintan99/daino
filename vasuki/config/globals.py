...
new directory and the same API key was pasted repeatedly, so opening Vasuki
...
_ENV_HOME = "VASUKI_CONFIG_HOME"


def global_config_dir() -> Path:
    """Directory holding user-level configuration and secrets.

    Honours ``VASUKI_CONFIG_HOME`` first, then ``XDG_CONFIG_HOME``, then the
    conventional ``~/.config``. Deliberately not ``~/.vasuki``: that path is a
    *project* marker, so writing user settings there would make the home
    directory look like a repository to every project-root search.
    """
