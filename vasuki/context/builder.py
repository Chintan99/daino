...
from vasuki.context.profiles import ModelExecutionProfile, adapt_context_bundle
from vasuki.memory import InstructionResolver, MemoryManager, MemoryType
from vasuki.repository import RepositoryIndexer
from vasuki.schemas import ContextBundle, TaskPacket, TaskSpec

MEMORY_PRECEDENCE = (
    "Authority: current repository/source code > current explicit user instruction > scoped "
    "VASUKI.md > active project decisions > verified project memory > episodic/session memory > "
...
class ContextBuilder:
    ...
    def _clip(value: str, token_limit: int) -> str:
        ...
        # Keep the tail because closest VASUKI.md and current user instructions
