"""Polished CLI surface for Vasuki's core engine."""
...
from vasuki.persistence.models import Approval, Checkpoint, Mission, Provider, Task
...
from vasuki.schemas import Message, MissionStatus, ProjectMode, RequirementSpec
...
def init(
    ...
    force: Annotated[
        bool, typer.Option("--force", help="Replace an existing Vasuki configuration.")
    ] = False,
) -> None:
...
async def _run_mission(request: str, mode: ProjectMode | None) -> None:
    root, settings, database = _context()
    service = MissionService(root, settings, database)
    with console.status("Running Vasuki mission…", spinner="dots"):
...
def missions_approve(mission_id: Annotated[str, typer.Argument()]) -> None:
    ...
    console.print(
        f"[green]Approved[/green] {mission_id}; run `vasuki missions resume {mission_id}`"
    )
